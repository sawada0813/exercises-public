...
describe("walk", () => {
  ...
  test("return files at root directory", async () => {
    ...
    for (let i = 0; i < expectedFiles.length; i++) {
      const value = dirent.next().value;
      let path = "";
      path += value.path.includes("./") ? "" : "./";
      path += `${value.path}/${value.name}`;
      files.push({ path, isDirectory: value.isDirectory() });
    }
  ...
  test("return file from very deep directory", () => {
    const addFiles = [
      ...
      {
        path: testFilePath + "/foo".repeat(5) + "/foo.txt",
        isDirectory: false,
      },
    ];
    const expectedFiles = fixedFiles.concat(addFiles);
